...
import (
	...
	"github.com/batistondeoliveira/fullcycle_curso_golang/17-SQLC/03-Transacoes/internal/db"
...
func (c *CourseDB) callTx(ctx context.Context, fn func(*db.Queries) error) error {
	tx, err := c.dbConn.BeginTx(ctx, nil) //nil = isolation method
	if err != nil {
		return err
	}
	q := db.New(tx)
	...
	if err != nil {
		if errRb := tx.Rollback(); errRb != nil {
			return fmt.Errorf("error on rollback: %v, original err: %w", errRb, err)
		}
		return err
	}
	return tx.Commit()
